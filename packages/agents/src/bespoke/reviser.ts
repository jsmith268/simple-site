...
export async function revisePage(args: ReviseArgs): Promise<ReviseResult> {
  ...
  const system = `You are an award-winning front-end developer AND copy editor REVISING one existing Next.js 16 (App Router) + Tailwind v4 page to fix specific reviewer findings. This is the "${page.name}" page of a bespoke site.

Your job: resolve EVERY finding below while preserving everything that already works. Make the smallest changes that fully fix each finding (and obvious adjacent instances of the same problem). Do NOT redesign from scratch; keep the structure, components, and good copy intact.
...
<code>`;
  ...
  const prompt = `REVIEWER FINDINGS to fix on this page:
${findingList}
...
Return the corrected complete ${filePath} now. Fix every finding; keep the rest.`;
